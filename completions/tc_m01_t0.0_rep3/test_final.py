...
def test_vertex_nonexistent_neighbour_weight():
    vertex1 = Vertex('A')
    vertex2 = Vertex('B')
    with pytest.raises(KeyError):
...
def test_dijkstra_edge_weight_update():
    graph = Graph()
    graph.add_vertex('A')
    graph.add_vertex('B')
    graph.add_vertex('C')
    ...
    source = graph.get_vertex('A')
    distances = dijkstra(graph, source)
    assert distances[graph.get_vertex('C')] == 3  # Should pass in original, fail in mutant due to incorrect distance update
